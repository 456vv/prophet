...
(function () {
	...
	var forEachEnumerableOwnProperty = function(obj, callback, excluded) {
		excluded = excluded || {};
		for (var k in obj) {
			if (!excluded[k]) {
	...
	var forEachEnumerableProperty = function(obj, callback, excluded) {
		excluded = excluded || {};
		for (var k in obj){
			if (!excluded[k]) {
				callback(k);
	...
	var prune = function (value, depthDecr, arrayMaxLength) {
		...
		function str(key, holder, depthDecr) {
			...
			case 'function':
				try{
					var v = replacer ? replacer(value, undefined, false) : value();
					if(v.constructor == Promise || typeof v == "Function"){
						return "new Function"
					}
					return String(v);
				}catch(e){
